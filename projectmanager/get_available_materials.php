...
try {
    // Fetch materials for dropdown - only show delivered materials with quantity > 0
    $query = "SELECT id, material_name, brand, specification, unit, material_price, labor_other, supplier_name, quantity as available_quantity
              FROM materials 
              WHERE status = 'Available' AND delivery_status = 'delivered' AND quantity > 0

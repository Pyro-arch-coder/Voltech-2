...
// First, get all users who are either clients, admins, or procurement with their latest message timestamp
$query = "SELECT 
    u.id, 
    u.firstname, 
    u.lastname, 
    ...
    u.user_level, 
    u.profile_path,
    CASE WHEN EXISTS (SELECT 1 FROM projects p WHERE p.client_email = u.email) THEN 1 ELSE 0 END as is_client,
    COALESCE(
    ...
    COALESCE(
        (SELECT MAX(date_sent) FROM admin_pm_messages 
         WHERE (sender_id = u.id AND receiver_id = ?) OR (sender_id = ? AND receiver_id = u.id)
         LIMIT 1),
        '1970-01-01 00:00:00'
    ) as last_admin_message
FROM users u
WHERE u.is_verified = 1
AND u.id != ? 
AND (EXISTS (SELECT 1 FROM projects p WHERE p.client_email = u.email) OR u.user_level IN (2, 4))";
...
if ($stmt = $con->prepare($query)) {
    ...
    $stmt->bind_param("iiiiiii", 
        $current_user_id, $current_user_id,  // client messages
        $current_user_id, $current_user_id,  // procurement messages
        $current_user_id, $current_user_id,  // admin messages
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $last_timestamps = [
            strtotime($row['last_client_message']),
            strtotime($row['last_procurement_message']),
            strtotime($row['last_admin_message'])
        ];
...
if ($unreadStmt = $con->prepare($unread_query)) {
    // We have 3 placeholders in the query, all using the same user ID
    $unreadStmt->bind_param("iii", $current_user_id, $current_user_id, $current_user_id);
    $unreadStmt->execute();

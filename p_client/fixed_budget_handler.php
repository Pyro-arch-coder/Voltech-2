...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $fixedAmount = isset($_POST['fixed_amount']) ? floatval($_POST['fixed_amount']) : 0;
        ...
        try {
            ...
            if (isset($_FILES['payment_proof']) && $_FILES['payment_proof']['error'] === UPLOAD_ERR_OK) {
                ...
                $stmt = $con->prepare("INSERT INTO proof_of_payments 
                    (project_id, user_id, file_name, file_path, status) 
                    VALUES (?, ?, ?, ?, 'pending')");
                $stmt->bind_param('iiss', $projectId, $userId, $fileName, $filePath);
            ...
            $stmt = $con->prepare("
                ...
                WHERE project_id = ?
            ");
            $stmt->bind_param('di', $fixedAmount, $projectId);
            ...
            if ($pmRow = $pmResult->fetch_assoc()) {
                ...
                $message = "Client has submitted an initial budget of ₱" . number_format($fixedAmount, 2) . " for project " . htmlspecialchars($project_name);
            ...
            $response = [
                'success' => true,
                'message' => 'Fixed budget and payment proof submitted successfully',
                'amount' => $fixedAmount,

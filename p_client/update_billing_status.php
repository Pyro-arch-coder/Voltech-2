...
try {
    ...
    // Update the billing request status
    $new_status = ($action === 'approve') ? 'approved' : 'rejected';
    $update_query = "UPDATE billing_requests SET status = ? WHERE id = ? AND project_id = ?";
    $update_stmt = $con->prepare($update_query);
    $update_stmt->bind_param("sii", $new_status, $billing_id, $project_id);
    
    if (!$update_stmt->execute()) {
        throw new Exception('Failed to update billing request status');
    }
    ...
    // Log the action
    error_log("Billing request {$billing_id} {$action} by client user {$user_id} for project {$project_id}");
